...
def plot_line(df, label, ax=None):
    ...
    color = p[0].get_color()
    slope, intercept = calc_slope_intercept((df['nodes'].iloc[0], df['ns/day'].iloc[0]),
                                            (df['nodes'].iloc[1], df['ns/day'].iloc[1]))
    ax.plot(df['nodes'], lin_func(df['nodes'], slope, intercept), ls='--', color=color, alpha=.5)
    return ax


def plot_over_group(df, groupby, ax=None):
    # plot all lines
    gb = df.groupby(groupby.split(','))
    for key, df in gb:
    ...
    ax2.set_xticklabels(gb.get_group(gb.groups[0])['ncores'])
...
def plot(csv, groupby):
    ...
    plot_over_group(df, groupby.split(','), ax=ax)
